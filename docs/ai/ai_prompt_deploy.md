...
## 9. Backlog: квоти Gemini та надійність AI-чату

### Зафіксоване спостереження

Під час перевірки локального й Render-середовища велика частина AI-запитів
повертала `429 RESOURCE_EXHAUSTED`/`503`. Причиною виявилися квоти попередньої
моделі, а не Render, Neon або API-ключ. Для навчального середовища обрано
`GEMINI_MODEL=gemini-flash-lite-latest` у локальному `.env` та Render
Environment Variables. Назву моделі не потрібно hard-code у Python і не слід
зберігати разом із секретами; вона лишається конфігураційною змінною.

На момент перевірки модель Flash Lite мала суттєво вищі доступні RPM/RPD, ніж
попередньо використаний Flash, тому підходить для демонстраційного навантаження.
Фактичні квоти залежать від Google-проєкту та можуть змінюватися; їх потрібно
перевіряти в AI Studio перед зміною моделі або політики retry.

### Запропонований наступний інкремент

| Пріоритет | Покращення | Очікуваний результат |
| --- | --- | --- |
| 1 | Розділити в API й UI причини тимчасової відмови: `gemini_unavailable`, `gemini_quota`, `chat_checkpoint_unavailable`. У логах зберігати лише code, attempt і correlation ID, без ключів, prompt-ів або фінансових даних. | Діагностика `429/503` без розкриття даних. |
| 2 | Для `429` не робити коротких повторів; враховувати `Retry-After`, якщо він наявний, або показувати час повторної спроби. Короткий retry лишити для `500/502/503/504` і мережевих збоїв. Додати окремий ліміт саме на вихідні виклики Gemini, а не лише на HTTP-запити користувача. | Один запит користувача не множить вичерпану quota. |
| 3 | Додати детермінований server-side preflight для типових запитів: порожній текст, відсутній обов'язковий період, некоректний діапазон дат, порожні дані за період. | Неповні дані не надсилаються в Gemini. |
| 4 | Для розпізнаних намірів («топ-витрати», «підсумок», «категорії», «динаміка») спершу виконувати контрольований read-only tool у Neon. Передавати Gemini лише непорожній мінімальний агрегат, коли потрібне текстове пояснення. | Менше LLM-викликів і нижче навантаження на quota. |
| 5 | Якщо Neon tool або LangGraph checkpointer недоступний, завершувати запит відповідним `503` одразу; не передавати Gemini технічний `data_unavailable` і не робити наступний LLM-крок. Перевірити політику оновлення з'єднань окремого psycopg pool checkpointer. | Збій БД не витрачає Gemini quota та не маскується. |
| 6 | Додати idempotency key/статус `failed` для user-повідомлення. Повторна спроба після `503` має використовувати те саме повідомлення, а не створювати дублікат в історії діалогу. | Чиста історія й безпечний retry для користувача. |
